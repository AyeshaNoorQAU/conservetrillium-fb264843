...
import { Loader2, LogOut, Trash2, Inbox, Megaphone, Settings, Plus, ShieldAlert, ArrowLeft, Save } from "lucide-react";
...
type Suggestion = { id: string; name: string | null; email: string | null; message: string; created_at: string; read_at: string | null };
      ...
      <div className="min-h-screen grid place-items-center px-6">
        <div className="max-w-md text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            ...
            <code className="px-1 mx-1 rounded bg-secondary">admin</code> role.
            Open the backend dashboard and add a row to <code className="px-1 rounded bg-secondary">user_roles</code> with your user id and role = <em>admin</em>.
          </p>
          <button onClick={() => supabase.auth.signOut().then(() => navigate({ to: "/login" }))} className="mt-6 text-xs underline text-muted-foreground">
            Sign out
          </button>
        </div>
      </div>
          ...
          {([
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm transition-all ${
                tab === t.id ? "bg-primary text-primary-foreground" : "bg-card text-foreground border border-border hover:border-primary/40"
              }`}
            >
...
function AnnouncementsAdmin() {
  ...
  return (
    <div className="grid lg:grid-cols-5 gap-6">
      <form onSubmit={post} className="lg:col-span-2 p-6 rounded-2xl bg-card border border-border h-fit">
        <h2 className="text-display text-xl mb-4">New announcement</h2>
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title"
          className="w-full mb-3 px-4 py-3 rounded-xl bg-background border border-border outline-none text-sm" maxLength={200} required />
        <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Write your update…" rows={6}
          className="w-full px-4 py-3 rounded-xl bg-background border border-border outline-none text-sm resize-none" maxLength={4000} required />
        <button disabled={saving} className="mt-4 inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-primary text-primary-foreground text-sm disabled:opacity-60">
          ...
          <div key={a.id} className="p-5 rounded-2xl bg-card border border-border">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex flex-col gap-2 shrink-0">
                <button onClick={() => togglePublish(a)} className="text-xs px-3 py-1.5 rounded-full border border-border hover:border-primary/40">
                  {a.published ? "Unpublish" : "Publish"}
                </button>
                <button onClick={() => remove(a.id)} className="text-xs text-ember inline-flex items-center gap-1 px-3 py-1.5 rounded-full border border-border hover:border-ember/40">
...
function SuggestionsAdmin() {
  ...
  const markRead = async (s: Suggestion) => {
    await supabase.from("suggestions").update({ read_at: s.read_at ? null : new Date().toISOString() }).eq("id", s.id);
    load();
  };
  const remove = async (id: string) => {
  ...
  return (
    <div className="space-y-3">
      ...
      {items.map((s) => (
        <div key={s.id} className={`p-5 rounded-2xl bg-card border ${s.read_at ? "border-border opacity-70" : "border-primary/40"}`}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[11px] uppercase tracking-widest text-muted-foreground">
                {new Date(s.created_at).toLocaleString()}
                {s.name && <> · {s.name}</>}
                {s.email && <> · <a className="text-primary" href={`mailto:${s.email}`}>{s.email}</a></>}
              </p>
              <p className="text-sm text-foreground whitespace-pre-wrap mt-2">{s.message}</p>
            </div>
            <div className="flex flex-col gap-2 shrink-0">
              <button onClick={() => markRead(s)} className="text-xs px-3 py-1.5 rounded-full border border-border hover:border-primary/40">
                {s.read_at ? "Mark unread" : "Mark read"}
              </button>
              <button onClick={() => remove(s.id)} className="text-xs text-ember inline-flex items-center gap-1 px-3 py-1.5 rounded-full border border-border hover:border-ember/40">
                <Trash2 className="w-3 h-3" /> Delete
              </button>
            </div>
          </div>
...
function SettingsAdmin() {
  ...
  useEffect(() => {
    supabase.from("site_settings").select("*").then(({ data }) => setSettings((data as Setting[]) ?? []));
  }, []);
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <p className="text-sm text-muted-foreground">
        Editable text snippets shown across the site. Add new keys here, then reference them in code.
      </p>
      {settings.map((s, i) => (
        <div key={s.key} className="p-5 rounded-2xl bg-card border border-border">
          ...
          <textarea
            ...
            rows={2}
            onChange={(e) => { const c = [...settings]; c[i] = { ...s, value: e.target.value }; setSettings(c); }}
          ...
          >
            {saving === s.key ? <Loader2 className="w-3 h-3 animate-spin" /> : <Save className="w-3 h-3" />} Save
          </button>
        </div>
      ))}
    </div>
